...
class PcaOnScaledData:
    ...
    def __init__(self, X, y, random_state: int = 42, test_size: float = 0.3, classifier = RandomForestClassifier, data_name: str = None):
        ...
        self.scaled_data = StandardScaler().fit_transform(X)
        ...
        self.scaled_accuracy = self._compute_accuracy(self.scaled_data)
    ...
    def component_vs_variance(self, additional_info: str = None) -> List[float]:
        ...
        variance_ratios_scaled = [e for e in self.PCA_object_scaled.explained_variance_ratio_]
        for i in range(1, self.n_features):
            variance_ratios[i] += variance_ratios[i - 1]
            variance_ratios_scaled[i] += variance_ratios_scaled[i - 1]

        # Plot
        plt.plot(variance_ratios, label="Unscaled data")
        plt.plot(variance_ratios_scaled, label="Scaled data")
        ...
        return variance_ratios, variance_ratios_scaled
    ...
    def component_vs_accuracy(self, debug: bool = False, limit: int = None, additional_info: str = None) -> List[float]:
        ...
        if debug:
            print(f"DEBUG:Start of this motherfucker")
        ...
        for i in range(1, limit):
            ...
            jj = self._compute_accuracy(self.scaled_data[:, :i])
            accuracies.append( ii )
            accuracies_scaled.append( jj )
            if debug:
                print(f"DEBUG:   Iter_{i}/{limit} : self._compute_accuracy(self.reduced_data[:, :{i}]) = {ii}")
                print(f"DEBUG:              : self._compute_accuracy(self.scaled_data[:, :{i}]) = {jj}")
        end = perf_counter()
        duration = end - start

        if debug:
            print(f"DEBUG:It took {duration:.2f} seconds to run this motherfucker")

        # Plot
        plt.plot(accuracies, label="Unscaled data")
        plt.plot(accuracies_scaled, label="Scaled data")
        ...
        plt.axhline(self.scaled_accuracy, color='g')
        plt.text(txt_pos, self.scaled_accuracy, "Scaled\nAccuracy",
                 horizontalalignment='center', verticalalignment='center', weight='bold', style="italic")
        plt.legend(loc="best")
        ...
        return accuracies, accuracies_scaled
